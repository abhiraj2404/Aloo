...
interface BillData {
    ...
    tableSession: {
        ...
        orders: { id: string; totalAmount: number; status: string; orderItems: { id: string; name: string; price: number; quantity: number }[] }[];
    };
}
        ...
        <div className="flex flex-col h-full">
            <div className="flex items-center justify-between py-3 border-b">
                <div className="flex items-center gap-2">
                    ...
                    {(statusCounts.GENERATED ?? 0) > 0 && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 font-semibold">
                            {statusCounts.GENERATED} Unpaid • ₹{Math.round(totalUnpaid / 100)}
                        </span>
                    )}
                </div>
            </div>
